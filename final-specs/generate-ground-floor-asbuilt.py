...
*** AS-BUILT VERSION — reflects actual measured column positions ***

Key differences from design:
  - C3/C4 at y=8.417 (was y=9.0) — moved 7" toward front
  - C6 center at x=6.0 (was 6.375) — shifted 4.5" left
  - C7 center at x=16.75 (was 17.375) — shifted 7.5" left
  - Front bay: 8'5" (was 9'3")
  - Back bay: 16'7.5" (was 15'6")
  - Staircase: tread reduced to 8.55" (was 9"), landing 2ft (was 2.5ft)
...
def build_ground_floor():
    ...
    # C3: left wall at y=8.417 (AS-BUILT: 8'5" from front, was 9'0" in design)
    MIDBEAM_Y = 8.417  # as-built position of C3/C4 row
    v, t = box_mesh(ft(-WALL_T), ft(MIDBEAM_Y - COL_12/2), ft(Z_PLINTH),
                    ft(COL_9), ft(COL_12), ft(COL_H))
    ...
    # C4: right wall at y=8.417 (AS-BUILT)
    v, t = box_mesh(ft(INT_W), ft(MIDBEAM_Y - COL_12/2), ft(Z_PLINTH),
                    ft(COL_9), ft(COL_12), ft(COL_H))
    ...
    # C6: AS-BUILT at x=5.625 (center x=6.0, measured 6'4.5" from C5 center)
    C6_X = 5.625  # center = 6.0
    v, t = box_mesh(ft(C6_X), ft(-WALL_T), ft(Z_PLINTH),
                    ft(COL_9), ft(COL_9), ft(COL_H))
    cb.add_geometry('col_C6', v, t, 'column')

    # C7: AS-BUILT at x=16.375 (center x=16.75, measured 10'9" from C6 center)
    C7_X = 16.375  # center = 16.75
    v, t = box_mesh(ft(C7_X), ft(-WALL_T), ft(Z_PLINTH),
                    ft(COL_9), ft(COL_9), ft(COL_H))
    cb.add_geometry('col_C7', v, t, 'column')

    # ============ 5. STAIRCASE (AS-BUILT ADJUSTED) ============
    # Front bay reduced from 9.25ft to 8.417ft (c/c). Beam face at ~7.79ft from front.
    # Adjusted: tread from 9" to 8.55", landing from 2.5ft to 2.0ft
    # Flight 1 (RIGHT side of stair zone, toward C6):
    #   x=3.25 to x=6 (2.75ft wide), y=0 to y=5.7 (5.7ft run)
    #   9 risers, 8" riser, 8.55" tread (8 treads × 8.55" = 5.7ft)
    #   Rises from z=3 to z=3+9*(8/12)=3+6=9

    RISER_H_FT = 8.0 / 12.0      # 8 inches = 0.667ft (unchanged)
    TREAD_D_FT = 8.55 / 12.0     # 8.55 inches = 0.7125ft (AS-BUILT: was 9"/0.75ft)
    FLIGHT_W = 2.75
    RISERS_PER_FLIGHT = 9
    TREADS_PER_FLIGHT = 8         # one less than risers
    FLIGHT_RUN = TREADS_PER_FLIGHT * TREAD_D_FT  # 8 × 0.7125 = 5.7ft
    LANDING_DEPTH = 2.0           # AS-BUILT: was 2.5ft, now 2.0ft (meets IS minimum)

    # Flight 1: x=3.25 to x=6, y=0 to y=5.7, rising from z=3 to z=9
    ...
    # Landing: x=0 to x=6 (full width), y=5.7 to y=7.7 (2.0ft deep), at z=9
    LANDING_Z = Z_PLINTH + RISERS_PER_FLIGHT * RISER_H_FT  # 3 + 6 = 9ft
    v, t = box_mesh(ft(0), ft(FLIGHT_RUN), ft(LANDING_Z),
                    ft(6.0), ft(LANDING_DEPTH), ft(RISER_H_FT))
    cb.add_geometry('stair_landing', v, t, 'staircase')

    # Flight 2 (LEFT side, from landing going back toward front):
    #   x=0 to x=2.75, starting at y=7.5 (landing back edge - 0.2ft turning zone)
    #   9 risers, rising from z=9 to z=15
    FLIGHT2_START_Y = FLIGHT_RUN + LANDING_DEPTH - 0.2  # 5.7 + 2.0 - 0.2 = 7.5
    v, t = stair_mesh(ft(0), ft(FLIGHT2_START_Y), ft(LANDING_Z),
    ...
    #   x=2.75 to x=3.25 (0.5ft), y=0 to y=7.7 (landing back), z=3 to z=9
    STAIR_ENCL_Y = FLIGHT_RUN + LANDING_DEPTH  # 7.7
    v, t = box_mesh(ft(2.75), ft(0), ft(Z_PLINTH),
                    ft(0.5), ft(STAIR_ENCL_Y), ft(LANDING_Z - Z_PLINTH))
    ...
    # Middle beam (9"x20"): spans x=0-20, at y=MIDBEAM_Y (AS-BUILT: 8.417)
    v, t = box_mesh(ft(0), ft(MIDBEAM_Y), ft(Z_SLAB_SOFFIT - BEAM_20),
                    ft(INT_W), ft(BEAM_9), ft(BEAM_20))
    ...
    # Stair well opening adjusted: y=0 to y=7.7 (was 8.5, now stops at beam face)
    STAIR_WELL_Y = STAIR_ENCL_Y  # 7.7ft (landing back edge = beam face zone)

    # Piece 1: BACK portion (y=STAIR_WELL_Y to y=25.75) — full width
    v, t = box_mesh(ft(-WALL_T), ft(STAIR_WELL_Y), ft(Z_SLAB_SOFFIT),
                    ft(INT_W + 2*WALL_T), ft(INT_D - STAIR_WELL_Y + WALL_T), ft(SLAB_T_FT))
    cb.add_geometry('roof_slab_back', v, t, 'slab')

    # Piece 2: FRONT-RIGHT portion (x=3 to x=20.75, y=-6 to y=STAIR_WELL_Y) — includes cantilever
    v, t = box_mesh(ft(3.0), ft(-CANTILEVER), ft(Z_SLAB_SOFFIT),
                    ft(INT_W - 3.0 + WALL_T), ft(STAIR_WELL_Y + CANTILEVER), ft(SLAB_T_FT))
    cb.add_geometry('roof_slab_front_right', v, t, 'slab')

    # Piece 3: FRONT-LEFT edge (x=-0.75 to x=0, y=-6 to y=STAIR_WELL_Y) — left wall thickness strip
    v, t = box_mesh(ft(-WALL_T), ft(-CANTILEVER), ft(Z_SLAB_SOFFIT),
                    ft(WALL_T), ft(STAIR_WELL_Y + CANTILEVER), ft(SLAB_T_FT))
    ...
    v, t = box_mesh(ft(-WALL_T), ft(MIDBEAM_Y - COL_12/2), ft(Z_1F_FLOOR),
                    ft(COL_9), ft(COL_12), ft(WALL_1F_H))
    ...
    v, t = box_mesh(ft(INT_W), ft(MIDBEAM_Y - COL_12/2), ft(Z_1F_FLOOR),
                    ft(COL_9), ft(COL_12), ft(WALL_1F_H))
    ...
    v, t = box_mesh(ft(18.5), ft(MIDBEAM_Y), ft(Z_1F_FLOOR + 2.5),
...
def main():
    ...
    output_dae = os.path.join(script_dir, 'ground-floor-asbuilt.dae')
    cb.save(output_dae)
    ...
    try:
        ...
        output_glb = os.path.join(script_dir, 'ground-floor-asbuilt.glb')
